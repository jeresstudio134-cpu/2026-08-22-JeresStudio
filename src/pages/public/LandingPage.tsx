...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  const handleNavPriceList = () => {
    if (onNavigatePriceList) onNavigatePriceList();
    else if (onNavigate) onNavigate("pricelist");
  };
  ...
  useEffect(() => {
    ...
    const fetchFreshData = async () => {
      try {
        ...
        console.error("LandingPage fetch fresh data error:", err);
      } finally {
        if (isMounted) {
          setLoading(false);
        }
