...
export const AdminFinance: React.FC<AdminFinanceProps> = ({ settings }) => {
  ...
  const balances: KantongBalances = useMemo(() => {
    // If transactions list is loaded, compute dynamically to guarantee 100% mathematical consistency
    if (transactions.length > 0) {
      ...
      transactions.forEach((tx) => {
        ...
        let k = (tx.kantong || "margin").toLowerCase().trim() as KantongKasType;
        if (!computed[k]) {
          const kat = (tx.kategori || "").toLowerCase();
          if (kat.includes("modal") || kat.includes("kulakan") || kat.includes("bahan")) k = "modal";
          else if (kat.includes("overhead") || kat.includes("operasional") || kat.includes("listrik") || kat.includes("wifi")) k = "overhead";
          else if (kat.includes("gaji saya") || kat.includes("owner") || kat.includes("pribadi")) k = "gaji_saya";
          else if (kat.includes("karyawan") || kat.includes("operator") || kat.includes("setting")) k = "gaji_karyawan";
          else k = "margin";
        }
        if (computed[k]) {
          if (tx.tipe === "masuk") {
    ...
    if (transactions.length > 0) {
      ...
      const breakdownPemasukan = Object.entries(incomeCatMap)
        .map(([kategori, val]) => ({
          ...
          total: val.total,
          count: val.count,
          percentage: totalPemasukan > 0 ? Math.round((val.total / totalPemasukan) * 100) : 0,
        }))
      ...
      const breakdownPengeluaran = Object.entries(expenseCatMap)
        .map(([kategori, val]) => ({
          ...
          total: val.total,
          count: val.count,
          percentage: totalPengeluaran > 0 ? Math.round((val.total / totalPengeluaran) * 100) : 0,
        }))
        .sort((a, b) => b.total - a.total);
  ...
  const handleSyncAndRefresh = async () => {
    setRefreshing(true);
    try {
      await api.syncDatabase().catch(() => {});
      await loadFinanceData();
      setSuccessMessage("Data kas & transaksi berhasil disinkronkan langsung dengan database Neon!");
      setTimeout(() => setSuccessMessage(""), 4000);
    } catch {
      await loadFinanceData();
    } finally {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3.5 bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-wrap lg:flex-nowrap items-center gap-2 shrink-0">
          ...
          <button
            ...
            title="Segarkan & Sinkronkan Data Kas dengan Database Neon"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? "animate-spin text-indigo-600" : ""}`} />
            <span>{refreshing ? "Sinkronisasi..." : "Sinkron / Segarkan"}</span>
          </button>
