...
export const AdminProducts: React.FC = () => {
  ...
  const [customSatuanName, setCustomSatuanName] = useState("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const resolvedSatuan = isCustomSatuan
      ? customSatuanName.trim().toLowerCase()
      : formData.satuan.trim().toLowerCase();

    if (isCustomSatuan && !customSatuanName.trim()) {
      setFormError("Nama satuan baru wajib diisi.");
      return;
    }
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      ...
                      className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:underline inline-flex items-center gap-1 cursor-pointer transition-colors"
                    >
                      {isCustomSatuan ? "← Pilih Satuan" : "+ Satuan Baru"}
                    </button>
                  </div>
