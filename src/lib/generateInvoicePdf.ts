...
export async function generateInvoicePDF(
  ...
  const items = order.items && order.items.length > 0
    ? order.items
    : [{ nama_item: "Item", qty: 1, satuan: "pcs", harga_satuan: order.subtotal || order.total, subtotal: order.subtotal || order.total }];
...
export async function generateSuratJalanPDF(
  ...
  autoTable(doc, {
    startY: currentY,
    margin: { left: marginX, right: marginX },
    head: [["NO", "NAMA BARANG / Item", "JUMLAH / QTY", "KETERANGAN"]],
    body: tableBody,
    theme: "grid",
    headStyles: {
      fillColor: [51, 65, 85],
