...
export async function generateInvoicePDF(
  ...
  if (paperFormat.startsWith("thermal")) {
    ...
    items.forEach((it) => {
      ...
      const dimText = it.panjang && it.lebar
        ? `${it.panjang}${it.dimensi_unit || "m"} × ${it.lebar}${it.dimensi_unit || "m"}${it.jumlah_lembar && it.jumlah_lembar > 1 ? ` (${it.jumlah_lembar} lbr)` : ""}`
        : "";
      const infoText = [dimText, it.catatan_item || ""].filter(Boolean).join(" • ");
      if (infoText) {
        doc.setFont("helvetica", "italic");
        doc.setFontSize(is80 ? 6.5 : 5.5);
        const infoLines = doc.splitTextToSize(infoText, pageWidth - marginX * 2);
        doc.text(infoLines, marginX, currentY);
        currentY += infoLines.length * 3;
        doc.setFontSize(is80 ? 7 : 6);
      }
  ...
  // Helper: gabungkan info ukuran (P x L) & catatan item jadi satu baris keterangan
  const getItemInfoText = (it: any): string => {
    const dimText = it.panjang && it.lebar
      ? `${it.panjang}${it.dimensi_unit || "m"} × ${it.lebar}${it.dimensi_unit || "m"}${it.jumlah_lembar && it.jumlah_lembar > 1 ? ` (${it.jumlah_lembar} lbr)` : ""}`
      : "";
    const catatanText = it.catatan_item || "";
    return [dimText, catatanText].filter(Boolean).join(" • ");
  };
    ...
    didParseCell: (data) => {
      if (data.section === "body" && data.column.index === 1) {
        ...
        if (rawItem) {
          ...
          const infoText = getItemInfoText(rawItem);
          if (infoText) {
            doc.setFont("helvetica", "italic");
            ...
            const infoH = infoLines.length * 2.8 * scale;
            const paddingH = 2.8 * scale * 2; // seimbang: 2.8mm atas, 2.8mm bawah

            neededHeight = paddingH + nameH + gapH + infoH;
          }
    ...
    didDrawCell: (data) => {
      ...
      if (data.section === "body" && data.column.index === 1) {
        ...
        if (rawItem) {
          ...
          // 2. Info Ukuran & Catatan Item (Font Lebih Kecil & Miring, Lurus Rata Kiri)
          const infoText = getItemInfoText(rawItem);
          if (infoText) {
            ...
            for (let i = 0; i < infoLines.length; i++) {
              doc.text(infoLines[i], textX, currentYText);
