...
export const DocumentPrintPreviewModal: React.FC<DocumentPrintPreviewModalProps> = ({
  ...
  const isLunasStatus = (order.status_bayar || "").toLowerCase() === "lunas";
  const sisa = Math.max(0, grandTotal - (isLunasStatus ? grandTotal : dp));
  const isLunas = isLunasStatus || sisa <= 0;
