...
app.post("/api/products", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Neon product persist error:", err);
  }
...
app.delete("/api/products/:id", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Neon product delete error:", err);
  }
...
app.post("/api/orders", authenticateToken, async (req: Request, res: Response) => {
  ...
  if (!items || !Array.isArray(items) || items.length === 0) {
    res.status(400).json({ error: "Order harus memiliki minimal 1 Iteman." });
    return;
  }

  const invalidItems = items.filter((it: any) => !it.nama_item || !String(it.nama_item).trim());
  if (invalidItems.length > 0) {
    res.status(400).json({ error: "Setiap Iteman harus memiliki Nama Item yang valid (tidak boleh kosong)." });
    return;
  }
  ...
  try {
    await persistOrder(newOrder, savedItems);
  } catch (err) {
    console.error("Neon order persist error:", err);
  }
...
app.delete("/api/orders/:id", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Neon order delete error:", err);
  }
...
app.post("/api/transactions", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Neon transaction persist error:", err);
  }
...
app.delete("/api/transactions/:id", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.error("Neon transaction delete error:", err);
  }
